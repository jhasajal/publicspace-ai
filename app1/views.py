...
def mood_detection_view(request):
    ...
    while True:
        ...
        try:
            ...
            if isinstance(result, list) and len(result) > 0:
                ...
                filtered_emotions = {key: emotion_scores[key] for key in ['happy', 'neutral', 'surprise'] if
                                     key in emotion_scores}

                # Display the dominant emotion on the frame
                cv2.putText(frame, f"Emotion: {dominant_emotion}", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0),
                            2, cv2.LINE_AA)
                ...
                for i, (emotion, score) in enumerate(filtered_emotions.items()):
                    ...
                    cv2.putText(frame, text, (50, 100 + i * 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2,
                                cv2.LINE_AA)
    ...
    return HttpResponse("Mood detection is running. Close the webcam window to stop.")

5
def fire_detection_feed(request):
    def generate_frames():
        # Open the camera (change the index if using a different camera)
        ...
        while True:
            ...
            for info in results:
                ...
                for box in boxes:
                    ...
                    # Only draw bounding boxes if confidence > 50%
                    if confidence > 50:
                        ...
                        cv2.putText(frame, label, (x1 + 8, y1 + 100),
